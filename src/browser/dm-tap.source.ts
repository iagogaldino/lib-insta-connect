/**
 * Fonte unica do IIFE que e injetado na pagina do Instagram Web para interceptar
 * DMs via monkey-patch do WebSocket. Consumido por:
 *  - page.evaluateOnNewDocument (via InstaConnect.startDmTap)
 *  - snippet Tampermonkey gerado em scripts/build-userscript.ts
 *  - copia e cola direto no DevTools Console
 *
 * O corpo deste template literal e JS puro. NAO use TS/ES modules/imports aqui.
 * NAO use crase (`) nem ${...} dentro do corpo - isso quebraria o template.
 */
export const DM_TAP_SOURCE = String.raw`
(() => {
